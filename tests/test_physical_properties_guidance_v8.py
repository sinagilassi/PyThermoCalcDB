...
class TestPhysicalPropertiesGuidanceV8(unittest.TestCase):
    def test_reduced_properties_scalar_array_and_invalid(self):
        ...
        self.assertEqual(calc_reduced_temperature(300.0, 600.0), 0.5)
    ...
    def test_critical_compressibility_and_acentric_factor(self):
        ...
        self.assertTrue(math.isclose(calc_critical_compressibility_factor(5.0e6, 2.0e-4, 400.0), expected))
        self.assertEqual(calc_acentric_factor_from_reduced_vapor_pressure(0.1), 0.0)
        self.assertTrue(np.allclose(
    ...
    def test_heat_capacity_polynomial_and_integrals(self):
        self.assertEqual(calc_heat_capacity_polynomial(300.0, 10.0, 0.1), 40.0)
        self.assertTrue(np.allclose(
            _calc_heat_capacity_polynomial([300.0, 400.0], 10.0, 0.1),
            [40.0, 50.0],
        ))
        self.assertEqual(
            calc_enthalpy_change_from_cp_polynomial(300.0, 400.0, 10.0, 0.1),
            10.0 * 100.0 + 0.1 / 2.0 * (400.0**2 - 300.0**2),
        )
        self.assertTrue(math.isclose(
            calc_entropy_change_from_cp_polynomial(300.0, 400.0, 10.0, 0.1),
    ...
    def test_phase_change_watson_alias(self):
        ...
        self.assertTrue(math.isclose(
            calc_heat_of_vaporization_watson(40000.0, 350.0, 300.0, 500.0),
            expected,
            rel_tol=1e-12,
        ))
    ...
    def test_hildebrand_solubility_helpers(self):
        ...
        self.assertTrue(math.isclose(
            calc_solubility_parameter_from_hvap_volume(40000.0, 300.0, 1.0e-4),
            math.sqrt(expected_du / 1.0e-4),
            rel_tol=1e-12,
        ))
        self.assertTrue(math.isclose(
